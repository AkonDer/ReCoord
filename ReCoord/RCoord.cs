...
namespace ReCoord
{
    public class RCoord
    {
        ...
        public double getAlf()
        {
            ...
            double dx = Xst - Xtr; // разница координат станции и точки упреждения катеты
            double dy = Yst - Ytr; // разница координат станции и точки упреждения катеты
            ...
            if (dy > 0)
            {
                return Math.Acos(dx / X);
            }                                       // возвращаем наш угол
            else return 360 - Math.Acos(dx / X);
        }
